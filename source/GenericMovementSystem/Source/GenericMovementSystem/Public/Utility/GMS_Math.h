...
public:
	...
	static EGMS_MovementDirection CalculateMovementDirection(float Angle, float ForwardHalfAngle, float AngleThreshold);
...
template <typename ValueType>
constexpr float UGMS_Math::RemapAngleForCounterClockwiseRotation(const ValueType Angle)
{
...
inline float UGMS_Math::LerpClamped(const float From, const float To, const float Alpha)
{
	return From + (To - From) * Clamp01(Alpha);
}

inline float UGMS_Math::LerpAngle(const float From, const float To, const float Alpha)
{
...
inline FRotator UGMS_Math::LerpRotator(const FRotator& From, const FRotator& To, const float Alpha)
{
	auto Result{To - From};
	Result.Normalize();
...
inline float UGMS_Math::Damp(const float DeltaTime, const float Smoothing)
{
...
inline float UGMS_Math::ExponentialDecay(const float DeltaTime, const float Lambda)
{
	// https://www.rorydriscoll.com/2016/03/07/frame-rate-independent-damping-using-lerp/
...
template <typename ValueType>
ValueType UGMS_Math::Damp(const ValueType& Current, const ValueType& Target, const float DeltaTime, const float Smoothing)
{
...
template <typename ValueType>
ValueType UGMS_Math::ExponentialDecay(const ValueType& Current, const ValueType& Target, const float DeltaTime, const float Lambda)
{
	return Lambda > 0.0f
...
template <>
inline FRotator UGMS_Math::Damp(const FRotator& Current, const FRotator& Target, const float DeltaTime, const float Smoothing)
{
	return Smoothing > 0.0f
...
template <>
inline FRotator UGMS_Math::ExponentialDecay(const FRotator& Current, const FRotator& Target, const float DeltaTime, const float Lambda)
{
	return Lambda > 0.0f
...
inline float UGMS_Math::DampAngle(const float Current, const float Target, const float DeltaTime, const float Smoothing)
{
	return Smoothing > 0.0f
...
inline float UGMS_Math::ExponentialDecayAngle(const float Current, const float Target, const float DeltaTime, const float Lambda)
{
	return Lambda > 0.0f
...
inline FRotator UGMS_Math::DampRotator(const FRotator& Current, const FRotator& Target, const float DeltaTime, const float Smoothing)
{
	return Damp(Current, Target, DeltaTime, Smoothing);
}

inline FRotator UGMS_Math::ExponentialDecayRotator(const FRotator& Current, const FRotator& Target,
                                                   const float DeltaTime, const float Lambda)
{
	return ExponentialDecay(Current, Target, DeltaTime, Lambda);
}

inline float UGMS_Math::InterpolateAngleConstant(const float Current, const float Target,
...
inline FVector UGMS_Math::ClampMagnitude01(const FVector& Vector)
{
...
inline FVector3f UGMS_Math::ClampMagnitude01(const FVector3f& Vector)
{
	const auto MagnitudeSquared{Vector.SizeSquared()};
...
inline FVector2D UGMS_Math::ClampMagnitude012D(const FVector2D& Vector)
{
	const auto MagnitudeSquared{Vector.SizeSquared()};
...
inline FVector2D UGMS_Math::RadianToDirection(const float Radian)
{
	float Sin, Cos;
...
inline FVector UGMS_Math::RadianToDirectionXY(const float Radian)
{
	float Sin, Cos;
	FMath::SinCos(&Sin, &Cos, Radian);
...
inline FVector2D UGMS_Math::AngleToDirection(const float Angle)
{
	return RadianToDirection(FMath::DegreesToRadians(Angle));
}

inline FVector UGMS_Math::AngleToDirectionXY(const float Angle)
{
	return RadianToDirectionXY(FMath::DegreesToRadians(Angle));
}

inline double UGMS_Math::DirectionToAngle(const FVector2D& Direction)
{
	return FMath::RadiansToDegrees(FMath::Atan2(Direction.Y, Direction.X));
}

inline double UGMS_Math::DirectionToAngleXY(const FVector& Direction)
{
	return FMath::RadiansToDegrees(FMath::Atan2(Direction.Y, Direction.X));
}

inline FVector UGMS_Math::PerpendicularClockwiseXY(const FVector& Vector)
{
	return {Vector.Y, -Vector.X, Vector.Z};
}

inline FVector UGMS_Math::PerpendicularCounterClockwiseXY(const FVector& Vector)
{
	return {-Vector.Y, Vector.X, Vector.Z};
}

inline double UGMS_Math::AngleBetweenSkipNormalization(const FVector& From, const FVector& To)
{
