...
#include "GMS_AnimLayer_Overlay_Stacks.generated.h"


struct FCachedAnimStateData;

USTRUCT(BlueprintType)
struct GENERICMOVEMENTSYSTEM_API FGMS_AnimData_StackedOverlays
{
	GENERATED_BODY()
...
public:
	 ...
	 * @attention If you are customising the animation blueprints, you can configure the mapping of state names to gameplay tags in the GMS main AnimInstance or any GMS_AnimLayer derived AnimInstances.
	 * 当该列表中的任何动画状态节点具有相关性时，将对动画叠加进行评估。
	 * @attention 如果你在自定义动画蓝图，您可以在 GMS 主AnimInstance或任何 GMS_AnimLayer派生AnimInstance中配置状态名称到游戏标记的映射。
	 */
	...
	TArray<FGMS_AnimData_Overlay> Overlays;
...
USTRUCT(BlueprintType)
struct FGMS_StackedOverlaySetting
{
	GENERATED_BODY()
	...
	TArray<FGMS_AnimData_StackedOverlays> StackedOverlays;
};


USTRUCT(BlueprintType)
struct GENERICMOVEMENTSYSTEM_API FGMS_OverlayStackState
{
	GENERATED_BODY()
...
class GENERICMOVEMENTSYSTEM_API UGMS_AnimLayerSetting_Overlay_Stack final : public UGMS_AnimLayerSetting_Overlay
{
	GENERATED_BODY()

public:
	UPROPERTY(EditAnywhere, Category="GMS", meta=(EditCondition=false, EditConditionHides))
	TMap<FGameplayTag, FGMS_StackedOverlaySetting> AcceleratedOverlayModes;

protected:
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Settings",meta=(TitleProperty="Tag"))
	TArray<FGMS_StackedOverlaySetting> OverlayModes;
 ...
 */
UCLASS(Abstract)
class GENERICMOVEMENTSYSTEM_API UGMS_AnimLayer_Overlay_Stack : public UGMS_AnimLayer
{
	GENERATED_BODY()
	friend UGMS_AnimLayerSetting_Overlay_Stack;
...
protected:
	...
	UPROPERTY()
	TObjectPtr<const UGMS_AnimLayerSetting_Overlay_Stack> PrevSetting{nullptr};
	...
	TArray<FGMS_AnimData_StackedOverlays> OverlayStacks;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="State")
	TArray<FGMS_OverlayStackState> OverlayStackStates;
