 ...
 * @attention Any class inherited from this class should only be linked to GMS_MainAnimInstance(The main anim instance).
 * 所有动画层的基类。
 * @注意从该类继承的任何类都只能链接到 GMS_MainAnimInstance（主动画实例）。
 */
...
protected:
	...
	UPROPERTY(Transient)
	TObjectPtr<UGMS_MovementSystemComponent> MSC;
