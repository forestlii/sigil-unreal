...
#include "SigilAnimNode_LayeredBoneBlend.generated.h"


// Layered blend (per bone); has dynamic number of blendposes that can blend per different bone sets
USTRUCT(BlueprintInternalUseOnly)
struct FSigilAnimNode_LayeredBoneBlend : public FAnimNode_Base
{
...
public:
	...
	FSigilInputBlendPose ExternalLayerSetup;
...
public:
	FSigilAnimNode_LayeredBoneBlend()
	...
	SIGILMOVEMENT_API virtual void Initialize_AnyThread(const FAnimationInitializeContext& Context) override;
	SIGILMOVEMENT_API virtual void CacheBones_AnyThread(const FAnimationCacheBonesContext& Context) override;
	SIGILMOVEMENT_API virtual void Update_AnyThread(const FAnimationUpdateContext& Context) override;
	SIGILMOVEMENT_API virtual void Evaluate_AnyThread(FPoseContext& Output) override;
	SIGILMOVEMENT_API virtual void GatherDebugData(FNodeDebugData& DebugData) override;
...
public:
	...
	SIGILMOVEMENT_API void RebuildPerBoneBlendWeights(const USkeleton* InSkeleton);

	// Check whether per-bone blend weights are valid according to the skeleton (GUID check)
	SIGILMOVEMENT_API bool ArePerBoneBlendWeightsValid(const USkeleton* InSkeleton) const;

	// Update cached data if required
	SIGILMOVEMENT_API void UpdateCachedBoneData(const FBoneContainer& RequiredBones, const USkeleton* Skeleton);
