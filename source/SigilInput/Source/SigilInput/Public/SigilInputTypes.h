...
#include "SigilInputTypes.generated.h"

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FSigilInputComponentSignature, UEnhancedInputComponent*, InputComponent);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FSigilInputBufferWindowStateChangedSignature, FGameplayTag, BufferWindowName, bool, State);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FSigilReceivedInputSignature, const FInputActionInstance&, ActionData, const FGameplayTag&, InputTag, ETriggerEvent, TriggerEvent);

enum class ETriggerEvent : uint8;
class USigilInputProcessor;
 ...
 */
UENUM()
enum class ESigilInputBufferType : uint8
{
	/**
 ...
 */
UENUM()
enum class ESigilInputProcessorExecutionType : uint8
{
	/**
 ...
 */
USTRUCT()
struct SIGILINPUT_API FSigilAllowedInput
{
	GENERATED_BODY()
	 ...
	 * 允许的输入标签。
	 */
	UPROPERTY(EditAnywhere, Category="GIPS|Input", meta=(Categories="InputTag,Sigil.Input.InputTag"))
	 ...
	 * 允许输入的相等比较运算符。
	 */
	friend bool operator==(const FSigilAllowedInput& Lhs, const FSigilAllowedInput& RHS)
	{
 ...
 */
USTRUCT()
struct SIGILINPUT_API FSigilInputBufferWindow
{
	GENERATED_BODY()
...
public:
	 ...
	 */
	UPROPERTY(EditAnywhere, Category="GIPS|Input")
	ESigilInputBufferType BufferType = ESigilInputBufferType::LastInput;
	...
	TArray<FSigilAllowedInput> AllowedInputs;
...
struct SIGILINPUT_API FSigilBufferedInput
{
	GENERATED_BODY()
	 ...
	 * 缓冲输入的相等比较运算符。
	 */
	friend bool operator==(const FSigilBufferedInput& Lhs, const FSigilBufferedInput& RHS)
	{
		return Lhs.InputTag == RHS.InputTag
	...
	FSigilBufferedInput() = default;

	/**
	 * Constructor for buffered input.
	 * 缓冲输入的构造函数。
	 */
	FSigilBufferedInput(const FGameplayTag& InputTag, const FInputActionInstance& ActionData, const ETriggerEvent TriggerEvent)
		: InputTag(InputTag),
	 ...
	 * 缓冲输入的复制构造函数。
	 */
	FSigilBufferedInput(const FSigilBufferedInput& Other)
	 ...
	 * 缓冲输入的赋值运算符。
	 */
	FSigilBufferedInput& operator=(const FSigilBufferedInput& Other)
	{
		if (this == &Other)
			return *this;
		InputTag = Other.InputTag;
 ...
 */
USTRUCT()
struct SIGILINPUT_API FSigilInputActionSetting
{
	GENERATED_BODY()
 ...
 */
USTRUCT()
struct SIGILINPUT_API FSigilInputTagRelationship
{
	GENERATED_BODY()
