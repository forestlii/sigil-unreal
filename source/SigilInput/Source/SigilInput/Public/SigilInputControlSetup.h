...
#include "SigilInputControlSetup.generated.h"
...
class SIGILINPUT_API USigilInputControlSetup : public UDataAsset
{
	GENERATED_BODY()
...
public:
	 ...
	 */
	void HandleInput(USigilInputSystemComponent* IC, const FInputActionInstance& ActionData, const FGameplayTag& InputTag, ETriggerEvent TriggerEvent) const;
	 ...
	 */
	bool CheckInput(USigilInputSystemComponent* IC, const FInputActionInstance& ActionData, const FGameplayTag& InputTag, ETriggerEvent TriggerEvent);
...
protected:
	 ...
	 */
	virtual bool InternalCheckInput(USigilInputSystemComponent* IC, const FInputActionInstance& ActionData, const FGameplayTag& InputTag, ETriggerEvent TriggerEvent);
	 ...
	 */
	TArray<TObjectPtr<USigilInputProcessor>> FilterInputProcessors(const FGameplayTag& InputTag, const ETriggerEvent& TriggerEvent) const;
	...
	TArray<TObjectPtr<USigilInputChecker>> InputCheckers;
	 ...
	 */
	UPROPERTY(EditAnywhere, Category="GIPS|Input")
	ESigilInputProcessorExecutionType InputProcessorExecutionType{ESigilInputProcessorExecutionType::MatchAll};
	...
	TArray<TObjectPtr<USigilInputProcessor>> InputProcessors;

	/**
	 * Enables debug logging for input events.
	 * 为输入事件启用调试日志。
	 * @attention Requires LogSigilInput to be set to VeryVerbose to take effect. 需要将LogSigilInput设置为VeryVerbose才能生效。
	 */
	 ...
	 */
	UPROPERTY(EditAnywhere, Category="GIPS|Debug", meta=(EditCondition="bEnableInputDebug", Categories="InputTag,Sigil.Input.InputTag"))
