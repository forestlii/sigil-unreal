...
#include "SigilTabListWidgetBase.generated.h"

class UDEPRECATED_SigilTabDefinition;
...
struct SIGILUI_API FSigilTabDescriptor
{
	GENERATED_BODY()
	...
	/**
	 * Button type for the tab, must implement SigilTabButtonInterface.
	 * 选项卡的按钮类型，必须实现SigilTabButtonInterface。
	 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="Tab Definition", meta = (MustImplement = "/Script/SigilUI.SigilTabButtonInterface", AllowAbstract = "false"))
...
class SIGILUI_API USigilTabButtonInterface : public UInterface
{
	GENERATED_BODY()
};
 ...
 * 选项卡按钮接收和更新标签信息的接口。
 */
class SIGILUI_API ISigilTabButtonInterface
{
	GENERATED_BODY()
...
public:
	...
	void SetTabDefinition(const UDEPRECATED_SigilTabDefinition* TabDefinition);
	...
	void SetTabLabelInfo(const FSigilTabDescriptor& TabDescriptor);
};
...
class SIGILUI_API USigilTabListWidgetBase : public UCommonTabListWidgetBase
{
	GENERATED_BODY()
...
public:
	...
	bool GetPreregisteredTabInfo(const FName TabNameId, FSigilTabDescriptor& OutTabInfo);
	...
	bool FindPreregisteredTabInfo(const FName TabNameId, FSigilTabDescriptor& OutTabInfo);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="GUIS|TabList")
	bool RegisterDynamicTab(const FSigilTabDescriptor& TabDescriptor);
	 ...
	 * 选项卡创建事件的本地委托。
	 */
	DECLARE_EVENT_TwoParams(USigilTabListWidgetBase, FOnTabContentCreatedNative, FName /* TabId */, UCommonUserWidget * /* TabWidget */);
...
protected:
	...
	TArray<TObjectPtr<UDEPRECATED_SigilTabDefinition>> TabDefinitions_DEPRECATED;
	...
	TArray<FSigilTabDescriptor> PreregisteredTabInfoArray;
	 ...
	 */
	UPROPERTY()
	TMap<FName, FSigilTabDescriptor> PendingTabLabelInfoMap;
