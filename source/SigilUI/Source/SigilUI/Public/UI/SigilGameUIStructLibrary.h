...
#include "SigilGameUIStructLibrary.generated.h"
 ...
 */
DECLARE_DYNAMIC_DELEGATE_OneParam(FSigilUIActionExecutedDelegate, FName, ActionName);
...
struct FSigilUIActionBindingHandle
{
	GENERATED_BODY()
 ...
 */
USTRUCT(BlueprintType)
struct FSigilUIContextBindingHandle
{
	GENERATED_BODY()

	FSigilUIContextBindingHandle()
	{
	};
	 ...
	 */
	FSigilUIContextBindingHandle(ULocalPlayer* InLocalPlayer, UClass* InContextClass);
 ...
 */
USTRUCT()
struct FSigilRootViewportLayoutInfo
{
	GENERATED_BODY()
	 ...
	 */
	UPROPERTY(Transient)
	TObjectPtr<USigilGameUILayout> RootLayout = nullptr;
	 ...
	 */
	UPROPERTY(Transient)
	TArray<TObjectPtr<USigilGameUIContext>> Contexts;
	 ...
	 */
	FSigilRootViewportLayoutInfo(ULocalPlayer* InLocalPlayer, USigilGameUILayout* InRootLayout, bool bIsInViewport)
