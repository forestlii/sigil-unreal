...
#include "SigilCameraMode_WithPenetrationAvoidance.h"
#include "Engine/Canvas.h"
#include "GameFramework/Pawn.h"
#include "Components/PrimitiveComponent.h"
#include "GameFramework/Controller.h"
#include "Engine/HitResult.h"
#include "SigilCameraAssistInterface.h"
#include "GameFramework/CameraBlockingVolume.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(SigilCameraMode_WithPenetrationAvoidance)

namespace SigilMovementCameraMode_WithPenetrationAvoidance_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}


USigilCameraMode_WithPenetrationAvoidance::USigilCameraMode_WithPenetrationAvoidance()
{
	PenetrationAvoidanceFeelers.Add(FSigilCameraPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FSigilCameraPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FSigilCameraPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FSigilCameraPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FSigilCameraPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FSigilCameraPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FSigilCameraPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void USigilCameraMode_WithPenetrationAvoidance::UpdatePreventPenetration(float DeltaTime)
{
	...
	ISigilCameraAssistInterface* TargetControllerAssist = Cast<ISigilCameraAssistInterface>(TargetController);
	...
	if (PPActorRootComponent)
	{
		...
		{
			for (ISigilCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void USigilCameraMode_WithPenetrationAvoidance::PreventCameraPenetration(bool bSingleRayOnly, const float& DeltaTime, const AActor* ViewTarget, FVector const& SafeLoc, FVector& CameraLoc,
                                                                        float& DistBlockedPct)
{
	...
	{
		FSigilCameraPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(SigilMovementCameraMode_WithPenetrationAvoidance_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void USigilCameraMode_WithPenetrationAvoidance::DrawDebug(UCanvas* Canvas) const
{
