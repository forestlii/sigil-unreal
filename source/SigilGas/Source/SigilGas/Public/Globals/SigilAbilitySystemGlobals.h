...
#include "SigilAbilitySystemGlobals.generated.h"

class ISigilAbilitySystemGlobalsEventReceiver;
 ...
 */
UCLASS()
class SIGILGAS_API USigilAbilitySystemGlobals : public UAbilitySystemGlobals
{
	GENERATED_BODY()
...
public:
	...
	static void RegisterEventReceiver(TScriptInterface<ISigilAbilitySystemGlobalsEventReceiver> NewReceiver);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "GGA|Globals", meta=(DefaultToSelf="NewReceiver"))
	static void UnregisterEventReceiver(TScriptInterface<ISigilAbilitySystemGlobalsEventReceiver> NewReceiver);
	...
	void InitAttributeSetDefaults(UAbilitySystemComponent* AbilitySystem, const FSigilAttributeGroupName& GroupName, int32 Level = 1, bool bInitialInit = false) const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category = "GGA|Globals")
	void ApplyAttributeDefault(UAbilitySystemComponent* AbilitySystem, FGameplayAttribute& InAttribute, const FSigilAttributeGroupName& GroupName, int32 Level) const;
...
private:
	 ...
	 */
	UPROPERTY()
	TArray<TScriptInterface<ISigilAbilitySystemGlobalsEventReceiver>> Receivers;
};
 ...
 */
UINTERFACE()
class SIGILGAS_API USigilAbilitySystemGlobalsEventReceiver : public UInterface
{
	GENERATED_BODY()
};
 ...
 * 全局技能系统事件接收器的实现类。
 */
class SIGILGAS_API ISigilAbilitySystemGlobalsEventReceiver
{
	GENERATED_BODY()
