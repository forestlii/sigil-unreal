...
#include "GCS_CollisionTraceInstance.generated.h"
 ...
 * 碰撞检测命中事件的委托。
 */
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FGCS_OnTraceHitSignature, const FHitResult&, HitResult);

/**
 * Delegate for trace state change events.
 * 碰撞检测状态更改事件的委托。
 */
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FGCS_OnTraceStateChangedSignature, bool, NewState);
...
class GENERICCOMBATSYSTEM_API UGCS_CollisionTraceInstance : public UGCS_ActorOwnedObject
{
	GENERATED_BODY()

	friend UGCS_CollisionSystemComponent;
