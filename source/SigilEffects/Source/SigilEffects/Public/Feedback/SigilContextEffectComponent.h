...
#include "SigilContextEffectsInterface.h"
#include "SigilContextEffectComponent.generated.h"
...
class SIGILEFFECTS_API USigilContextEffectComponent : public UActorComponent, public ISigilContextEffectsInterface
{
	GENERATED_BODY()
...
public:
	 ...
	 */
	virtual void PlayContextEffectsWithInput_Implementation(FSigilSpawnContextEffectsInput Input) override;
	 ...
	 * @param Input The context effects input data. 情景效果输入数据。
	 */
	void AggregateContexts(FSigilSpawnContextEffectsInput& Input) const;
	 ...
	 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category="Settings", meta=(Categories="Sigil.Effects.SurfaceType"))
	 ...
	 */
	UPROPERTY(EditAnywhere, Category="Settings")
	TSet<TSoftObjectPtr<USigilContextEffectsLibrary>> DefaultContextEffectsLibraries;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="GES|ContextEffect")
	void UpdateLibraries(TSet<TSoftObjectPtr<USigilContextEffectsLibrary>> NewContextEffectsLibraries);
...
private:
	 ...
	 */
	UPROPERTY(Transient)
	TSet<TSoftObjectPtr<USigilContextEffectsLibrary>> CurrentContextEffectsLibraries;
