...
#include "SigilCollisionSystemComponent.generated.h"
 ...
 */
USTRUCT()
struct SIGILCOMBAT_API FSigilTraceInstancesCache
{
	GENERATED_BODY()
	...
	TArray<TObjectPtr<USigilCollisionTraceInstance>> Instances;
};
...
class SIGILCOMBAT_API USigilCollisionSystemComponent : public UPawnComponent
{
	GENERATED_BODY()
...
public:
	 ...
	 * 默认构造函数。
	 */
	USigilCollisionSystemComponent(const FObjectInitializer& ObjectInitializer);
	...
	static USigilCollisionSystemComponent* GetCollisionSystemComponent(const AActor* Actor);
	...
	static bool FindCollisionSystemComponent(const AActor* Actor, USigilCollisionSystemComponent*& Component);
...
protected:
	...
	TArray<TObjectPtr<USigilCollisionTraceInstance>> TraceInstances;
	...
	TMap<TSubclassOf<USigilCollisionTraceInstance>, FSigilTraceInstancesCache> CachedTraceInstances;
	 ...
	 */
	USigilCollisionTraceInstance* GetOrCreateTraceInstance(TSubclassOf<USigilCollisionTraceInstance> TraceClass);
...
public:
	...
	TArray<USigilCollisionTraceInstance*> CreateTraceInstances(TArray<FSigilCollisionTraceDefinition> Definitions, UPrimitiveComponent* PrimitiveComponent);
	...
	TArray<USigilCollisionTraceInstance*> GetTraceInstances() const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "GCS|Collision")
	void GetTraceInstanceByTag(FGameplayTag TraceToFind, USigilCollisionTraceInstance*& FoundTrace);
	 ...
	 */
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "GCS|Collision")
	void GetTraceInstanceByClass(TSubclassOf<USigilCollisionTraceInstance> TraceToFind, USigilCollisionTraceInstance*& FoundTrace);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "GCS|Collision")
	void RemoveTraceFromCreatedTraces(USigilCollisionTraceInstance* TraceToRemove);
	 ...
	 * 碰撞检测实例命中事件的委托。
	 */
	DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FSigilOnTraceInstanceHitSignature, USigilCollisionTraceInstance*, TraceInstance, const FHitResult&, HitResult);
	...
	FSigilOnTraceInstanceHitSignature OnTraceInstanceHitEvent;

	/**
	 * Delegate for trace instance state change events.
	 * 碰撞检测实例状态更改事件的委托。
	 */
	DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FSigilOnTraceInstanceStateChangedSignature, USigilCollisionTraceInstance*, TraceInstance, bool, bNewState);
	 ...
	 */
	UPROPERTY(BlueprintAssignable, BlueprintCallable, Category = "GCS|Combat")
	FSigilOnTraceInstanceStateChangedSignature OnTraceInstanceStateChangedEvent;
...
public:
	...
	void OnTraceInstanceHit(USigilCollisionTraceInstance* TraceInstance, const FHitResult& HitResult);
	 ...
	 */
	UFUNCTION(BlueprintNativeEvent, Category = "GCS|Collision")
	void OnTraceInstanceStateChanged(USigilCollisionTraceInstance* TraceInstance, bool bNewState);
	
	//谁碰到了我
	UPROPERTY(BlueprintAssignable, BlueprintCallable, Category = "GCS|Combat")
	FSigilOnTraceInstanceHitSignature OnBeTraceInstanceHitEvent;
	
	UFUNCTION(BlueprintNativeEvent, Category = "GCS|Collision")
	void OnBeTraceInstanceHit(USigilCollisionTraceInstance* TraceInstance, const FHitResult& HitResult);
};