...
#include "SigilCombatFlow.generated.h"

class USigilAttackResultProcessor;
...
class SIGILCOMBAT_API USigilCombatFlow : public USigilActorOwnedObject
{
	GENERATED_BODY()
...
public:
	 ...
	 * @注意 需要将SigilAbilitySystemGlobals设置为默认AbilitySystemGlobals。
	 ...
	 */
	UFUNCTION(BlueprintCallable, BlueprintNativeEvent, Category="GCS|Combat Flow")
	void HandleGameplayEffectExecute(const FSigilGameplayEffectModCallbackData& Payload);
	virtual void HandleGameplayEffectExecute_Implementation(const FSigilGameplayEffectModCallbackData& Payload);
	...
	void HandleAttackResult(const FSigilAttackResult& Payload);
...
protected:
	...
	TObjectPtr<USigilCombatSystemComponent> CombatComponent;
